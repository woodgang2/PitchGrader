    ...
    if team_name != '':
        ...
        if (df.empty):
            ...
            if not show_changes and show_color and stuff_df.shape[0] < 1000:
                stuff_df = stuff_df.style.applymap(color_values, subset = colored_columns).format("{:,.0f}", subset = colored_columns + ['PitchCount'])
                stuff_df['Fastball%'] = stuff_df['Fastball%'].apply(lambda x: format(x, '.2f'))
            container = st.container()
            container.markdown("<div margin-left: auto, margin-right: auto>", unsafe_allow_html=True)
            container.dataframe(stuff_df)
            container.markdown("</div>", unsafe_allow_html=True)
